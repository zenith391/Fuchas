...
local function pstr(val)
	local frm = string.format("%x", val)
	if frm:len() < 2 then
		frm = "0" .. frm
	end
	return frm
...
-- Edit by zenith391 for string output
local function squeeze(st)
	...
	for y = 1, 5 do
		for x = 1, 5 do
			if offset < blockWords then
				...
				local val = st[x][y]
				local vals = {bit32.band(val, 0x000000FF), bit32.rshift(bit32.band(val, 0x0000FF00), 8),
					bit32.rshift(bit32.band(val, 0x00FF0000), 16), bit32.rshift(bit32.band(val, 0xFF000000), 24)}
				str = str .. pstr(vals[1])
				str = str .. pstr(vals[2])
				str = str .. pstr(vals[3])
				str = str .. pstr(vals[4]) .. "|"
	...
	--	str = str .. string.format("%x", string.byte(v))
	--end
	return str
end
...
local function keccakHash(rate, length, data)
	...
	absorb(state, data)
	return squeeze(state):sub(1,length/4)
end
