...
-- OCIF default format
local ocif = {}
-- huge palette necessary for OCIF
...
ocif.decode = function(self, file)
	local method = string.byte(file:read(1))
	local image = {}
	if method == 5 then
		image.width = io.fromunum(file:read(2))
		image.height = io.fromunum(file:read(2))
		for i=1, image.width*image.height do
			image.backgrounds[i] = palette[string.byte(file:read(1))+1]
			image.foregrounds[i] = palette[string.byte(file:read(1))+1]
			file:read(1) -- skip alpha, not yet used
			image.chars[i] = file:read(1)
		end
	end
	return image
end
lib.registerImageFormat(ocif)