-- Edited version of skeeto's Lua uuid generator
-- Original: https://gist.github.com/skeeto/c61167ff0ee1d0581ad60d75076de62f#file-uuid-lua
...
for i = 0, 255 do
	...
	hex_hi[i] = ('0123456789abcdef'):byte(1 + bit32.rshift(i, 4))
end

local function binToHex(id)
	local f, r = string.format, string.rep
	return f(f("%s-%s%s", r("%.2x", 4), r("%.2x%.2x-", 3), r("%.2x", 6)), id:byte(1, 16))
end

local function hexToBin(addr)
	addr = addr:gsub("%-", "")
	local result = ""
	for i=1, #addr, 2 do
...
local function uuid(bin)
	local bytes = {}
	for i = 1, 16 do
		bytes[i] = squeeze()
	end
	if bin then
		local str = uuid(false)
		...
		return string.char(
			...
			hex_hi[bytes[ 8]],
			0x2d,
			('89ab'):byte(1 + bit32.band(bytes[9], 0x3)),
			hex_hi[bytes[ 9]],
			hex_lo[bytes[10]],
			hex_hi[bytes[10]],
			0x2d,
