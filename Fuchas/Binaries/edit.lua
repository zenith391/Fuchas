-- Port of LoganDark's ShEdit for Fuchas
...
local filename = term.resolve(args[1])
...
local function loadConfig()
	...
	local env = {}
	local config = loadfile("A:/etc/edit.cfg", nil, env)
	if config then
		pcall(config)
	end
	-- Fill in defaults.
...
local function setStatus(value)
	local x, y, w, h = 1, 1, gpu.getResolution()
...
local function setCursor(nbx, nby)
	...
	nbx, nby = getCursor()
	gpu.set(x + w - 10, y + h, string.format("%d,%d", nby, nbx))
end
...
local function find()
	...
	while running do
		...
		if address == component.screen.getKeyboards()[0] then
...
local function jump()
	...
	while true do
		...
		if address == component.getPrimary("screen").getKeyboards()[0] then
		...
		if type(keybinds) == "table" and keyBindHandlers[command] then
			for _, keybind in ipairs(keybinds) do
				if type(keybind) == "table" then
					...
					local keyboardAddress = term.getKeyboard()
...
while running do
	...
	if address == term.getKeyboard() or address == term.getScreen() then
		local blink = true
